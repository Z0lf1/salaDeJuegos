...
export class PreguntadosComponent implements OnInit {
  ...
  usuario: string = '';
  puntajes: any[] = []; // ✅ Puntajes solo de este juego
  ...
  async cargarPuntajes() {
    try {
      this.puntajes = await this.puntajesService.obtenerPuntajesPorJuego('Preguntados');
    } catch (error) {
  ...
  async guardarPuntaje() {
    try {
      ...
      this.cargarPuntajes(); // ✅ Actualiza la tabla tras guardar
    } catch (error) {
