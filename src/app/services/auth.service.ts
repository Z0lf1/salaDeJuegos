...
export class AuthService {
  ...
  async register(email: string, password: string): Promise<any> {
    const { data, error } = await this.supabase.auth.signUp({ email, password });
    if (error) {
      // Normalizar y traducir errores comunes de duplicado/registro
    ...
    if (insertError) {
      ...
      if (insertMsg.includes('duplicate') || insertMsg.includes('unique') || insertMsg.includes('already exists') || insertMsg.includes('violates unique')) {
        throw new Error('El usuario ya está registrado');
      }
      throw insertError;
    }
